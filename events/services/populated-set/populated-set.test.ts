...
describe('PopulatedSetService', () => {
  ...
  describe('getEventClass', () => {
    it('should throw if invalid searchTerm', async () => {
      await expect (PopulatedSetService.getPopulatedSet('random',
        'id', 0, 0)).rejects.toThrow()
    });
  });
  ...
  describe('check service works', () => {
    it('should retrieve correct SET by event type', async () => {
      ...
      const set = await PopulatedSetService.getPopulatedSet('accountDisabled', uri,
             1000, 2000, 'reason')
    ...
    it('should retrieve correct SET by URI', async () => {
      ...
      const set = await PopulatedSetService.getPopulatedSet(RiscEventURIs[RiscEventTypes.AccountDisabled].uri,
      uri, 1000, 2000, 'reason')
