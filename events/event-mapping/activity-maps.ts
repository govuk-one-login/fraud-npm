...
async function generateActivityUserSessionEvents(
...
export const activityPopulatedEventsMapping: Record<
  string,
  (
    id: string,
    startTimeInMillis: number,
...
> = {
  [ActivityEventURIs[ActivityEventTypes.SessionRecovered].uri]: async (
    id: string,
    startTimeAsLong: number,
    endTimeAsLong: number,
    ...args: (string | null)[]
  ) => {
    let events = await generateActivityUserSessionEvents(
    ...
    let event =
      events[ActivityEventURIs[ActivityEventTypes.SessionRecovered].uri];

    addStandardEventFields(
      event,
      startTimeAsLong,
      args[2] ?? DEFAULT_INITIATING_ENTITY,
      args[3] ?? DEFAULT_REASON_ADMIN,
      args[4] ?? DEFAULT_REASON_USER
    );
