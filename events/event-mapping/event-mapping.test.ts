import { EVENT_DETAILS_URI, EVENT_METADATA_URI, generateStandardUserSubjectEvents } from './events-mapping';
...
export async function validateSetEvents(setEvents: SETEvents, type: AllEventTypes, schema: Schema, logSet: boolean = false) {
  ...
  const details = setEvents[EVENT_DETAILS_URI + AllEventURIs[type].detailsKey + '/eventDetails'];
...
describe('generate functions', () => {
  ...
  it('generateStandardUserSubjectEvent works with event timestamp', async () => {
    ...
    const set = await generateStandardUserSubjectEvents(ActivityEventTypes.SessionRecovered,
    ...
    const expectedSet = {
      ...
      'https://vocab.account.gov.uk/secevent/v1/eventMetadata': { event_timestamp_ms: 100 },
  ...
  it('generateStandardUserSubjectEvent works with event timeframe, no end time', async () => {
    ...
    const set = await generateStandardUserSubjectEvents(ActivityEventTypes.SessionRecovered,
      'id', TimestampTypes.timeFrame, 100, 0)

    const expectedSet = {
      'https://vocab.account.gov.uk/secevent/v1/activity/sessionRecovered': { subject: { format: 'uri', uri: 'id' } },
      'https://vocab.account.gov.uk/secevent/v1/eventMetadata': { event_timeframe_ms : { start_time: 100 } },
      'https://vocab.account.gov.uk/secevent/v1/sessionRecovered/eventDetails': { location: 'GB' }
    }
  ...
  it('generateStandardUserSubjectEvent works with event timeframe, no start time', async () => {
    ...
    const set = await generateStandardUserSubjectEvents(ActivityEventTypes.SessionRecovered,
      'id', TimestampTypes.timeFrame, 0, 1000)

    const expectedSet = {
      'https://vocab.account.gov.uk/secevent/v1/activity/sessionRecovered': { subject: { format: 'uri', uri: 'id' } },
      'https://vocab.account.gov.uk/secevent/v1/eventMetadata': { event_timeframe_ms : { end_time: 1000 } },
      'https://vocab.account.gov.uk/secevent/v1/sessionRecovered/eventDetails': { location: 'GB' }
    }
