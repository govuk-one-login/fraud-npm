...
describe('generate functions', () => {
  ...
  it('check populated events conforms to schemas', async () => {
    ...
    const set: SETEvents = await subjectFn(DEFAULT_URI, 1000, 0,
        SESSION_ID, PREV_SESSION_ID, INITIATING_ENTITY, ADMIN_REASON, USER_REASON);

    await validateSetEvents(set, ActivityEventTypes.SessionRecovered, sessionRecoveredEventSchema);
  });
