...
const caepTestCases: TestInfo[] = [{
  type: CaepEventTypes.AssuranceLevelChange, schema: assuranceLevelChangeSchema,
  extraArgs: ['admin', 'reason-admin', 'reason-user', 'increase', 'nist-aal1', 'nist-aal2'],
}, {
  type: CaepEventTypes.CredentialChange, schema: credentialChangeSchema,
  extraArgs: ['admin', 'reason-admin', 'reason-user', 'create', 'password', 'fido',
    'friendly-name', 'x509-issuer', 'x509-serial'],
}, {
  type: CaepEventTypes.DeviceComplianceChange, schema: deviceComplianceChangeSchema,
  ...
  type: CaepEventTypes.SessionRevoked, schema: sessionRevokedSchema,
  ...
  type: CaepEventTypes.TokenClaimsChange, schema: tokenClaimsChangeSchema,
...
describe('populated CAEP events', () => {
  ...
  it('check populated CAEP events conforms to schemas', async () => {
    ...
    for (let testCase of caepTestCases) {
      ...
      const set: SETEvents = await subjectFn(DEFAULT_URI, 100, 100, ...extraArgs);
